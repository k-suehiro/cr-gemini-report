...
- **アプリ利用判定の修正**: 一覧アイコンとアプリ別アクティブ数を、キャッシュの利用フラグだけでなく詳細履歴の実回数からも判定
- **アプリ名の正規化**: 大文字・小文字の違いを吸収し、`workflows`・`chat`・`calendar` の集計漏れを防止

### v1.1.5（2026-07-28）

- **アプリ別ユーザー一覧**: 集計アプリのセレクターを追加し、全アプリ・Gmail除く・各アプリを切り替え可能に
- **選択アプリ連動**: ユーザー別の利用回数・使用日数・活用レベル・並び替え・部門別集計を選択アプリに合わせて再計算

### v1.1.4（2026-07-28）

- **対応アプリ追加**: `workflows`・`chat`・`calendar` を集計・表示対象に追加
- **ユーザー一覧**: 3アプリの利用状況アイコンと、ホバー時の利用回数表示に対応
- **個人詳細モーダル**: 上部のアプリ別利用回数へ3アプリを追加
- **アプリ別集計**: アプリ別アクティブ数へ3アプリを追加
